    ...
    async function authenticateAndVerify() {
      ...
      try {
        ...
        const userNameSpan = document.getElementById('userName'); if (userNameSpan) userNameSpan.textContent = gitUsername;
        const sidebarName = document.getElementById('sidebarName'); if (sidebarName) sidebarName.textContent = gitUsername;
        const sidebarRole = document.getElementById('sidebarRole'); if (sidebarRole) sidebarRole.textContent = 'Connected to GitHub';
        const showLoginBtn = document.getElementById('showLoginBtn'); if (showLoginBtn) showLoginBtn.style.display = 'none';
        const logoutBtn = document.getElementById('logoutBtn'); if (logoutBtn) logoutBtn.style.display = 'flex';
        const statusDot = document.getElementById('statusDot'); if (statusDot) statusDot.classList.add('connected');
        const statusText = document.getElementById('statusText'); if (statusText) statusText.textContent = 'Connected';
        updateUIBasedOnAuth();
    ...
    function logout() {
      ...
      const userNameSpan = document.getElementById('userName'); if (userNameSpan) userNameSpan.textContent = 'Guest';
      const sidebarName = document.getElementById('sidebarName'); if (sidebarName) sidebarName.textContent = 'Guest';
      const sidebarRole = document.getElementById('sidebarRole'); if (sidebarRole) sidebarRole.textContent = 'Not logged in';
      const showLoginBtn = document.getElementById('showLoginBtn'); if (showLoginBtn) showLoginBtn.style.display = 'flex';
      const logoutBtn = document.getElementById('logoutBtn'); if (logoutBtn) logoutBtn.style.display = 'none';
      const statusDot = document.getElementById('statusDot'); if (statusDot) statusDot.classList.remove('connected');
      const statusText = document.getElementById('statusText'); if (statusText) statusText.textContent = 'Disconnected';
      updateUIBasedOnAuth();
      const sidebarAvatar = document.getElementById('sidebarAvatar'); if (sidebarAvatar) sidebarAvatar.src = 'https://i.ibb.co.com/chGXxvw1/avt.jpg';
      navigateTo('home');
    ...
    async function executeCreateRepo() {
      ...
      try {
        ...
        const newRepoName = document.getElementById('newRepoName'); if (newRepoName) newRepoName.value = '';
        const repoDesc = document.getElementById('repoDesc'); if (repoDesc) repoDesc.value = '';
        await loadRepositories();
    ...
    document.getElementById('refreshReposBtn')?.addEventListener('click', () => { if (!isAuthenticated) { showToast('Please login first', 'warning'); return; } const btn = document.getElementById('refreshReposBtn'); if (btn) btn.innerHTML = '<i class="fas fa-spinner fa-pulse"></i>'; loadRepositories().finally(() => { if (btn) btn.innerHTML = '<i class="fas fa-sync-alt"></i> Refresh'; }); });
    document.getElementById('repoSearchInput')?.addEventListener('input', () => { renderRepositories(); });
    
    // CLONE FUNCTIONALITY
    function showCloneModal(repoName) { const repo = allRepositories.find(r => r.name === repoName); if (repo) { currentCloneTarget = repo; const urlText = document.getElementById('cloneUrlText'); if (urlText) urlText.textContent = repo.clone_url; const cmdText = document.getElementById('cloneCommandText'); if (cmdText) cmdText.textContent = repo.clone_url; showModal('cloneModal'); } }
